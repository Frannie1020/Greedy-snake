...
void over()
{
	...
	if(Snake1.life==0&&Snake2.life==0){
        if (Snake1.jieshu>Snake2.jieshu) {
            ...
            paintText(50,150,"VIOLET WIN");
        } else if (Snake1.jieshu<Snake2.jieshu) {
			setTextColor(RGB(64, 224, 208));
            paintText(30,150,"	Turquoise WIN");
        } else {
            setTextColor(GREEN);
            paintText(100,150,"EQUAL!!!");
        }
...
void paint()
{
    ...
    setBrushColor(GREEN);
    ellipse(Food.x,Food.y,Food.x+10,Food.y+10);

	setBrushColor(RGB(238, 130, 238));  // Snake1
	setPenColor(RGB(238, 130, 238));
    for (i=0;i<Snake1.jieshu;i++) {
    	ellipse(Snake1.x[i],Snake1.y[i],Snake1.x[i]+10,Snake1.y[i]+10);
	}
	setPenColor(WHITE);  // Snake1 Tail
	setBrushColor(WHITE);
	ellipse(Snake1.x[Snake1.jieshu-1], Snake1.y[Snake1.jieshu-1], Snake1.x[Snake1.jieshu-1]+10, Snake1.y[Snake1.jieshu-1]+10);

	setBrushColor(RGB(64, 224, 208));  // Snake2
	setPenColor(RGB(64, 224, 208));
    for (i=0;i<Snake2.jieshu;i++) {
		chrod(Snake2.x[i], Snake2.y[i], Snake2.x[i] + 10, Snake2.y[i] + 10, 5,5,5,5);
	}
	setPenColor(WHITE);   // Snake2 Tail
	setBrushColor(WHITE);
	chrod(Snake2.x[Snake2.jieshu - 1], Snake2.y[Snake2.jieshu - 1], Snake2.x[Snake2.jieshu - 1] + 10, Snake2.y[Snake2.jieshu - 1] + 10, 5,5,5,5);
	endPaint();
}
